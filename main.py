...
def showLogin():
    ...
    g_client_id = json.loads(
        open('secret/g_client_secrets.json', 'r').read())['web']['client_id']
    fb_app_id = json.loads(
        open('secret/fb_client_secrets.json', 'r').read())['web']['app_id']
    state = ''.join(random.choice(
                    string.ascii_uppercase + string.digits)for x in xrange(32))
    
    login_session['state'] = state
    login_session['g_client_id'] = g_client_id
    login_session['fb_app_id'] = fb_app_id
...
def fbconnect():
    """Handler for LOGIN via facebook."""
    ...
    app_secret = json.loads(
        open('secret/fb_client_secrets.json', 'r').read())['web']['app_secret']
    ...
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
    login_session['image_url'] = data["data"]["url"]

    # See if user exists
    user_id = getUserID(login_session['email'])
    if not user_id:
        user_id = createUser(login_session)
        flash("note: both your name and password are set to your email.")
...
def fbdisconnect():
    """Handler for DISCONNECTING from facebook."""
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets(
            'secret/g_client_secrets.json', scope='')
    ...
    login_session['name'] = data['name']
    login_session['image_url'] = data['picture']
    login_session['email'] = data['email']
